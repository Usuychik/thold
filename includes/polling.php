...
function thold_update_host_status () {
			...
			if (!empty($fh['host_id'])) {
				...
				if ($host['status'] == HOST_UP) {
                    ...
                    if ($alert_phone == '' && $host['thold_send_sms'] > 0) {
						cacti_log('THOLD: Can not send a Host Recovering SMS since the \'Alert SMS\' setting is not set !', true, 'POLLER');
					} elseif ($host['thold_send_sms'] == '0') {
						cacti_log('THOLD: Did not send a Host Recovering SMS, disabled per host setting : ' . $host['description'] . ' !', true, 'POLLER');
					} elseif ($alert_phone != '') {
						thold_sms($alert_phone, $subject);
						cacti_log('THOLD: Sent SMS ' . $host['description'] . ' !', true, 'POLLER');
                    }
	...
	if (count($hosts)) {
		foreach($hosts as $host) {
			...
			if ($alert_phone == '' && $host['thold_send_sms'] > 0) {
				cacti_log('THOLD: Can not send a Host Down SMS since the \'Alert SMS\' setting is not set !', true, 'POLLER');
			} elseif ($host['thold_send_sms'] == '0') {
				cacti_log('THOLD: Did not send a Host Down SMS, disabled per host setting : ' . $host['description'] . ' !', true, 'POLLER');
			} elseif ($alert_phone != '') {
				thold_sms($alert_phone, $subject);
			}
		}
